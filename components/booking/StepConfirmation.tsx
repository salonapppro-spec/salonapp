    ...
    <div className="space-y-4">
      ...
      {email ? (
        <p className="text-xs text-neutral-400">
          Потвърждението ще бъде изпратено на <strong className="text-neutral-200">{email}</strong> &mdash; проверете дали е изписан вярно.
        </p>
      ) : null}
      {emailSuggestion ? (
        <p className="rounded-lg bg-amber-400/20 px-3 py-2 text-xs font-medium text-amber-300">
          &#9888; Възможна грешка в имейла &mdash; имахте предвид <strong>{emailSuggestion}</strong>? Върнете се назад, за да го коригирате.
        </p>
      ) : null}
      <p className="text-xs text-neutral-400">Натиснете &bdquo;Потвърди записването&ldquo;, за да изпратите заявката.</p>
    </div>
  );
}
