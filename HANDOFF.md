...
## 2026-08-21 — Таня: върнати разстоянията по списъците (CSS специфичност)

Лина поиска повече въздух между текстовите карти и снимките в „Обучения“. Причината се оказа по-широка: reset-ът беше `.t-site ul, .t-site dl { margin: 0 }` — специфичност (0,1,1), която бие класовите правила (0,1,0) по-долу. **Всеки `margin-top` по списък мълчаливо се зануляваше.** Засегнати бяха `.t-train-shots`, `.t-book-hours`, `.t-contact-list` и `.t-request-list` — и четирите връщаха `marginTop: 0px` на живия сайт.

Поправка: reset-ът минава на `:where(...)`, което има нулева специфичност, така че класовете печелят по ред. Разстоянието в „Обучения“ е вдигнато допълнително на `clamp(2rem, 4.5vw, 3.4rem)`.

След поправката (десктоп 1425): карти → снимки 54px (беше 26), работно време 35px, контакти 32px, „След консултация“ 26px. На 375px: 32px.

---

## 2026-08-21 — Таня: само евро, разделени мъжки и дамски пакети

- **Левът е махнат отвсякъде** — цените са само в евро (страната вече е в еврозоната). Премахнати са `BGN_RATE` и `bgnLabel` от `data.ts`, левовият ред под цената в ценоразписа и левът в подсказката на формата за резервация. Проверено на живо: `document.body.innerText` не съдържа „ лв“.
- **Махнат е подзаглавието** под „Цени и времетраене“ (то обясняваше левовата равностойност).
- **Пакетните услуги са в две отделни групи, мъжките първи**: „Мъжко подстригване с измиване и стайлинг“ (3 услуги) и „Дамско подстригване с измиване и стайлинг“ (2). Регексът `^Мъжко (под|фейд)` продължава да хваща и правописната грешка „подсригване“ от админ панела.
- Нов тест в `tests/tania-services.test.ts` заключва реда на групите и че нищо не пада в „Други услуги“ (190 теста).
