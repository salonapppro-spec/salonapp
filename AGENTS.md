# SalonApp.pro — Правила за Codex

> Четат се от Codex автоматично при стартиране на сесия.
...
**Деплой и тест (критично):** Кодът се деплойва към Vercel от **оригиналния репозиторий** (`origin` — каноничният remote, вързан за production). **Не** разчитаме на локални worktree-и или копия за „истинския“ deploy. След merge/push, проверката и QA са срещу **реалния публичен домейн** (напр. salonapp.pro и салонските URL), не само `localhost` — екипът деплойва в оригинала и тества в реалния домейн.
